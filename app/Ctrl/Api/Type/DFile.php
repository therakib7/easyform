...
class File
{
    ...
    {
        register_rest_route("rhef/v1", "/files", [
            [
                "methods" => "GET",
                "callback" => [$this, "get"],
        ...
        register_rest_route("rhef/v1", "/files/(?P<id>\d+)", [
            "methods" => "GET",
        ...
        register_rest_route("rhef/v1", "/files/(?P<id>[0-9,]+)", [
            "methods" => "DELETE",
    ...
    public function create($req)
    {
        ...
        if (empty($tab_id)) {
            $reg_errors->add(
                "field",
                esc_html__("Tab ID is missing", "easyform")
            );
        }
        ...
        if (empty($type)) {
            $reg_errors->add(
                "field",
                esc_html__("Type is missing", "easyform")
            );
        }
        ...
        if ($reg_errors->get_error_messages()) {
            ...
            if (!is_wp_error($post_id)) {
                update_post_meta($post_id, "ws_id", rhef()->get_workspace());
    ...
    {
        return current_user_can("rhef_file");
    }
