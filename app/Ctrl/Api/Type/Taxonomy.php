...
class Taxonomy
{
    ...
    {
        register_rest_route("rhef/v1", "/taxonomies", [
            [
                "methods" => "GET",
                "callback" => [$this, "get"],
        ...
        register_rest_route("rhef/v1", "/taxonomies/(?P<id>\d+)", [
            "methods" => "GET",
    ...
    public function get($req)
    {
        ...
        if (empty($taxonomies)) {
            $reg_errors->add(
                "field",
                esc_html__("Taxonomy is missing", "easyform")
            );
        }
        ...
        if ($reg_errors->get_error_messages()) {
            ...
            foreach ($taxonomies as $taxonomy) {
                ...
                if ($id) {
                    $tags = get_the_terms($id, "rhef_" . $taxonomy);
                    $tagList = [];
                    if ($tags) {
                        foreach ($tags as $tag) {
                            $color = get_term_meta(
        ...
        if ($reg_errors->get_error_messages()) {
            ...
            if ($reorder) {
                ...
                $add_taxonomy = wp_insert_term(
                    $label, // the term
                    "rhef_" . $taxonomy // the add_taxonomy
                );
        ...
        if ($reg_errors->get_error_messages()) {
            ...
            if ($add) {
                ...
                if ($post_id) { //delete term from post
                    wp_remove_object_terms($post_id, $term_id, 'rhef_' . $taxonomy);
                } else { // delete term
                    // wp_delete_term($term_id, 'rhef_' . $taxonomy);
                }
                wp_send_json_success();
            } else {
                $add_taxonomy = wp_update_term(
                    $term_id,   // the term
                    'rhef_' . $taxonomy, // the taxonomy
                    array(
                        'name' => $label,
                    )
                );
    ...
    {
        return current_user_can("rhef_taxonomy");
    }
